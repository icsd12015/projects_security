 ...
 */
public final class SIP_Client implements Runnable {
    ...
    public SIP_Client (String address, int port, int localPort) throws IOException, SecurityException {
        ...
        //Sundesi tou socket ston proxy
        this.sock = new Socket(InetAddress.getByName(address), port, InetAddress.getByName("localhost"), localPort);

        this.out = new ObjectOutputStream(sock.getOutputStream());
        this.out.flush();
        this.in = new ObjectInputStream(sock.getInputStream());

        //To address einai i local dieuthinsi tou socket  (px An LocalSocketAddress == localhost/127.0.0.1:5060 -> address = 127.0.0.1:5060 )
        this.address = sock.getLocalSocketAddress().toString().split("/")[1];
        //To realm antistoixa an px LocalSocketAddress == localhost/127.0.0.1:5060 -> realm = 127.0.0.1 )
        this.realm = sock.getRemoteSocketAddress().toString().split("/")[1].split(":")[0];
    ...
    public void run () {
        ...
        while (keepRunning) {
            try {
                ...
                System.out.println("");
                ColorPrint.print("Client at: " + this.sock.getLocalSocketAddress()
                        + " Received an Encrypted SIP Packet from Proxy at: " + sock.getRemoteSocketAddress(), ColorPrint.ANSI_CYAN);
    ...
    public void handle (SIP_Packet packet) {
        switch (packet.getMethod()) {
            case INVITE: //An to SIP paketo exei methodo INVITE
                ...
                if (JOptionPane.showOptionDialog(ontop, "Contact " + packet.getFromHeader() + " is calling", "Incoming call",
