 ...
 */
public class SIP_TorProxy implements Runnable {
    ...
    private final TorServerSocket torServerSocket;
    ...
    public SIP_TorProxy (String hostname, int port, int hiddenServicePort, String registrar_url, String logsFile) throws IOException, NotBoundException {
        this.clients = new HashMap<>();
        this.logsFile = logsFile;
        //Pairnw tin anafora sto Registrar vasi tou url me RMI
        this.dbserver = (DBServInterface) Naming.lookup(registrar_url);

        //Bind tou ServerSocket me Tor Relay
        JavaTorRelay node = new JavaTorRelay(new File(logsFile));
        this.torServerSocket = node.createHiddenService(port, hiddenServicePort);

        this.serverSock = torServerSocket.getServerSocket();
        ColorPrint.print("Started server at: " + serverSock.getInetAddress().getHostAddress() + ":" + port + ".", ColorPrint.ANSI_GREEN);
        ColorPrint.print("Hidden Service Binds to: " + torServerSocket.getFullAddress() + ".\n", ColorPrint.ANSI_GREEN);

        this.realm = torServerSocket.getHostname();
    }
    ...
    public void addClient (Socket sock) {
        System.out.println("");
        ColorPrint.print("Proxy at: "
                + this.serverSock.getLocalSocketAddress()
                + "/" + this.torServerSocket.getFullAddress()
                + " accepted connection from: " + sock.getRemoteSocketAddress().toString().split("/")[1], ColorPrint.ANSI_CYAN);
    ...
    public void handle (String ID, SIP_Packet request) {
...
//                            clients.get(ID).address = request.peekLastViaHeader()[0];
            ...
            case INVITE: //An to SIP paketo exei methodo INVITE
                ...
                if ((!localInstances && (request.getFromHeader().split("@")[1]).equals(this.realm)) || (localInstances && request.getViaHeaders().size() == 1)) {
                    if (!request.hasAuthorizationHeader()) { //An to sip paketo periexei Authorization Header
                        ...
                        if (authorize(request, ID)) { //An epituxei to authorization vasi twn stoixeiwn tou Authorization Header kai twn stoixeiwn tou registered client
                            clients.get(ID).address = request.peekLastViaHeader()[0];//Dimiourgia sip paketou me code 100 TRYING vasi tou paketou pou lifthike
        ...
        try {
            ...
            if (connection == null) { //An de uparxei idi sundesi me ton Proxy toy paralipti
                ...
                OnionProxyManager onionProxyManager = new JavaOnionProxyManager(
                        new JavaOnionProxyContext(new File(logsFile + "/ClientProxy" + logsFile.charAt(logsFile.length() - 1))));
                ...
                Socket pipe = Utilities.socks4aSocketConnection(request.getToHeader().split("@")[1], 5060, "127.0.0.1", localOnionPort);
    ...
    public String getHostName () {
        return this.torServerSocket.getHostname();
    }

    public int getPort () {
        return this.torServerSocket.getServicePort();
    }
    ...
    class UAS_Thread extends Thread {
        ...
        UAS_Thread (Socket socket) {
            ...
            this.IP = pipe.getRemoteSocketAddress().toString().split("/")[1].split(":")[0];
            ...
            while (keepRunning) {
                try {
                    ...
                    ColorPrint.print("Proxy at: "
                            ...
                            + "/" + torServerSocket.getFullAddress()
